...
void idAI::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( attacker->IsType( idPlayer::GetClassType() ) ) {
		enemykiller = static_cast <idPlayer*>(attacker);
		enemykiller->inventory.exp += 1;
		if( enemykiller->inventory.exp < 50 ) {
			enemykiller->inventory.correctarmor = 10;
		}
		else if( enemykiller->inventory.exp < 100 ) {
			enemykiller->inventory.correctarmor = 20;
		}
		else if( enemykiller->inventory.exp < 150 ) {
			enemykiller->inventory.correctarmor = 30;
		}
		else if( enemykiller->inventory.exp < 200 ) {
			enemykiller->inventory.correctarmor = 40;
		}
		else if( enemykiller->inventory.exp < 250 ) {
			enemykiller->inventory.correctarmor = 50;
		}
		else if( enemykiller->inventory.exp < 300 ) {
			enemykiller->inventory.correctarmor = 60;
		}
		else if( enemykiller->inventory.exp < 350 ) {
			enemykiller->inventory.correctarmor = 70;
		}
		else if( enemykiller->inventory.exp < 400 ) {
			enemykiller->inventory.correctarmor = 80;
		}
		else if( enemykiller->inventory.exp < 450 ) {
			enemykiller->inventory.correctarmor = 90;
		}
		else {
			enemykiller->inventory.correctarmor = 100;
		}
	}
	aifl.dead = true;
