...
void idInventory::GetPersistantData( idDict &dict ) {
	...
	//correctarmor
	dict.SetInt( "correctarmor", correctarmor );
...
void idInventory::RestoreInventory( idPlayer *owner, const idDict &dict ) {
	...
	correctarmor	= dict.GetInt( "correctarmor", "0" );
	exp				= dict.GetInt( "exp", "0" );
...
void idInventory::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteInt( correctarmor );
...
void idPlayer::UpdateHudStats( idUserInterface *_hud ) {
	...
	if (temp != inventory.exp ) {
		...
		_hud->SetStateFloat	( "player_exppct", idMath::ClampFloat ( 0.0f, 1.0f, (float)inventory.exp / (float)inventory.maxEXP ) );
...
void idPlayer::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir,
	...
	if ( inventory.correctarmor != inventory.armor )
	{
		inventory.armor = inventory.correctarmor;
	}
